...
public class ProducerCallBackRouteBuilder extends RouteBuilder {

    @Override
    public void configure() throws Exception {
        from("timer:tick?period={{kafka.time.period}}")
                .setBody( exchange -> new MyEvent(UUID.randomUUID().toString(), "event-" + new Random().nextInt(1000)+1, "test", System.currentTimeMillis()))
